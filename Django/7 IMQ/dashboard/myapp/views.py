...
def login(request):    
    if request.method == 'POST':
        ...
        if (login_user.count()) == 1:
            ...
            request.session['role'] = login_user.get().role
...
def add_admin(request):
    ...
    return render(request,'add_admin.html',{'branch':branch})
...
def update_admin(request,id):
    ...
    branch = Branch.objects.all()
    return render(request,'add_admin.html',{'data':data,'branch':branch})
...
def add_inquiry(request):
    ...
    inq = Admin.objects.all()
    return render(request,'add_inquiry.html',{'branch':branch,'course':course,'user':user,'inq':inq})

def view_inquiry(request):
    loinrole = request.session.get('role')
    loginUser = Admin.objects.filter(email=request.session.get('email'))
    if loinrole == 'SuperAdmin':
        data = Inquiry.objects.all()
    else:
        print("ok")
        data = Inquiry.objects.filter(branch=loginUser.get().branch)        
        print(data)
...
def update_inquiry(request,id):
    ...
    course = Course.objects.all()
    inq = Admin.objects.all()
    return render(request,'add_inquiry.html',{'data':data,'branch':branch,'course':course,'inq':inq})
